...
for i in direstelarmasses:
    if (i=='galaxies_specs_ch_1.3.csv'):
        tabelas.append(i)
    if (i=='nomes.csv'):
        nomes=pd.read_csv(i)
...
'''
tabela_slope_massa['Diferença massas']=abs(tabela_slope_massa['1.3']-tabela_mdyn['log_M'])
tabela_slope_massa['Nomes']=tabela_slope_massa.index


plt.figure()
tabela_slope_massa.plot.scatter(x='Nomes', y='Diferença massas')

'''
ax1=tabela_slope_massa.plot(kind='scatter', x='Slopes', y=nomes[0], title='Gráfico para galáxias "mal comportadas"', color=cores[0], label=nomes[0])
for i in range(1, len(nomes)):
    ax2=tabela_slope_massa.plot(kind='scatter', x='Slopes', y=nomes[i], title='Gráfico para galáxias "mal comportadas"', color=cores[i],label=nomes[i], ax=ax1)
...
plt.xlabel("Nomes dos espectros")
plt.ylabel("|LogM calculado - LogM conhecido|")
plt.savefig('g_ch.jpg', dpi=400)

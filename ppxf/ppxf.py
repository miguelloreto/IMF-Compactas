...
class ppxf(object):
    ...
    def __init__(self, templates, galaxy, noise, velScale, start,
        ...
        for j in range(self.ncomp):
            ...
            vl = start1[0] + np.array([-1e3,1e3])/velScale # +/-2000 km/s from first guess (MT: modified to 1000 km/s)
            parinfo[0+p]['limits'] = vlims[2*j:2*j+2] = vl
            parinfo[0+p]['step'] = 1e-2
            parinfo[1+p]['value'] = start1[1]
            parinfo[1+p]['limits'] = np.array([0.1, 1e3/velScale]) # hard-coded velScale/10<sigma<1000 km/s  (MT: modified to 500 km/s)

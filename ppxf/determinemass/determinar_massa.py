...
for i in outputs:
    ...
    if (i[-1] == 'out'):
        if(i[0] == 'spec-0303-51615-0278'):
            ...
            df=pd.read_csv(selecionado, index_col=0, skiprows=34,names=['j','x_j', 'M_ini', 'Mcor', 'age', 'met', 'L/M', 'Mstar'], nrows=112, delim_whitespace=True, usecols=[0,1,2,3,4,5,6,7])
            ...
            mcor.append(float(linhas[11].split(" ")[-1]))
            fator_norm_ppxf.append(float(linhas[8].split(" ")[-1]))
            df=pd.read_csv(selecionado, index_col=0, skiprows=40,names=['j','x_j', 'M_ini', 'Mcor', 'age', 'met', 'L/M', 'Mstar'], nrows=112, delim_whitespace=True, usecols=[0,1,2,3,4,5,6,7])
            lista_df_espectros.append(df)
            nome_espectros.append(selecionado)
    else:
        continue

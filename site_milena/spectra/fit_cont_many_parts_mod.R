...
nsig_low_b = vector(length = Nparts); nsig_low_b[] = 0.5
degree_b = vector(length = Nparts); degree_b[] = 1
span_b = vector(length = Nparts); span_b[] = 0.5
Nit_b = vector(length = Nparts); Nit_b[] = 5

# nsig_up_b[c(1, 4, 5, 6, 8, 12)] = 1.2
# span_b[c(3, 4, 5)] = 0.9
parts_to_correct = 0 #c(1 : Nparts)
factor_number = 0.037
...
xx.xx = spec_in$V1 < lmax
print(length(flux_corr$V1[xx.xx]))
print(length(flux_corr$V2[xx.xx]))
...
legend('bottomright', c(leg1, leg2, leg3, leg4), col = c('black', 'gray', 'red', 'red'), lty = c(1, 1, 1, 5), bty = 'n', cex = 1.2)

# -----------------
# AXIS
# -----------------
axis(1, seq(6000, 30000, 50), tcl = -0.6, labels = T)
axis(1, seq(6000, 30000, 10), tcl = -0.3, labels = F)
...
plot(t$lambda, t$flux_corr, type = 'l', log = 'y', # xlim = c(9500, 11000), ylim = c(0.1, 0.2),
     xlab = expression(paste(lambda, ' (', ring(A), ')')), ylab = 'Flux', xaxt = 'n')
lines(t$lambda, t$flux_cont_corr, col = 'red', lwd = 0.5)

leg1 = 'Flux-corrected spectra'
leg2 = 'Continuum fit'
legend('bottomright', c(leg1, leg2), col = c('black', 'red'), lty = 1, bty = 'n', cex = 1.2)
